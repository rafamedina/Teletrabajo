...
- [ ] **Task: Update Model Fields (TDD)**
  - [ ] Write tests to verify the existence and behavior of `attachment_ids` (Many2many to `ir.attachment`) and `reason` (Text).
  - [ ] Implement the fields in `models/telework_request.py`.
- [ ] **Task: Security & ACLs**
  - [ ] Verify that the `ir.model.access.csv` and record rules allow users to manage their own attachments.
- [ ] **Task: Conductor - User Manual Verification 'Phase 1: Model & Backend Logic' (Protocol in workflow.md)**
...
- [ ] **Task: Define Calendar View (TDD)**
  - [ ] Write tests (Tours or Python) to verify the presence of the new calendar view definition.
